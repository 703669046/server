/*! v5.0.0.9 | pageoffice.js for java | (c) 2016, 2020 Beijing zhuozheng zhiyuan software, Inc.*/
...
var POBrowser = {
    ...
    openWindow: function(strURL, strOptions,obj, strArgument) {
        ...
        zsxmlhttp.onload = function() {
            ...
            if (strData.indexOf('"name":"jsonx"') > 0) {
                if (parseInt(zsxmlhttp.responseText[7]) < 5) {
        ...
        zsxmlhttp.setRequestHeader('Authorization',obj.token);
        zsxmlhttp.setRequestHeader('#version#','#pc');
        zsxmlhttp.open(obj.type, PO_code + "json.htm?x=" + po_uuid(8, 16));
        zsxmlhttp.send()
    },
    openWindowModeless: function(strURL, strOptions, obj, strArgument) {
        if(Object.keys(obj).length==0){
            obj.type='POST';
            token="0"
        }
            ...
            if (zsxmlhttp.responseText.indexOf('"name":"jsonx"') > 0) {
                ...
                po_ajax({
                    ...
                    success: function(data) {
                        ...
                        zsxmlhttp2.send("token=" + strToken + "&open=" + encodeURIComponent(PO_datas[0] + POParent + "|" + strToken + "|") + "&parent=" + POParent + "&Info=" + PO_datas[1])
                    }
                })
            }
        };
